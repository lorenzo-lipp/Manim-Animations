...
class Presentation(Scene):
    def construct(self):
        ...
        self.wait(0.3)
        self.play(Indicate(text_1, color=LIGHT_RED_COLOR))
        self.play(Indicate(text_2, color=LIGHT_RED_COLOR))
        self.wait(1.5)
        self.play(pieces[6].animate(run_time=1.5, rate_func=lambda t: smooth(t if t < 0.5 else (1 - t), 10)).scale(1.4).set_color(YELLOW))
        self.play(pieces[3].animate(run_time=1.5, rate_func=lambda t: smooth(t if t < 0.5 else (1 - t), 10)).scale(1.4).set_color(YELLOW))
