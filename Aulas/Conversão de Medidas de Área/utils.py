...
SQUARE_SIZE = 2
UNITS = [
    ["km^2", "Quilômetro", LIGHT_RED_COLOR], 
    ["ham^2", "Hectômetro", DISABLED_COLOR], 
    ["dam^2", "Decâmetro", DISABLED_COLOR], 
    ["m^2", "Metro", LIGHT_BLUE_COLOR], 
    ["dm^2", "Decímetro", DISABLED_COLOR], 
    ["cm^2", "Centímetro", LIGHT_PURPLE_COLOR], 
    ["mm^2", "Milímetro", LIGHT_GREEN_COLOR]
]
        ...
        super().__init__(
            *[VGroup(
                ...
                VGroup(
                    Rectangle(unit[2], SQUARE_SIZE/3, SQUARE_SIZE, fill_opacity=0.2),
                    VGroup(
                        Tex(unit[1], color=unit[2]).scale(0.45),
                        Tex("Quadrado", color=unit[2]).scale(0.45)
...
class UnitsTableLeftArrow(VGroup):
    def __init__(self, table, from_index, to_index):
        super().__init__(
            ...
            Tex(f"x {100 ** (to_index - from_index):,}".replace(",", "."), color=UNITS[from_index][2])
...
class UnitsTableRightArrow(VGroup):
    def __init__(self, table, from_index, to_index):
        super().__init__(
            ...
            Tex(f"$\\div$ {100 ** (from_index - to_index):,}".replace(",", "."), color=UNITS[from_index][2])
