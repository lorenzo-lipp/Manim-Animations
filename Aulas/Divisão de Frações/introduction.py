...
class Introduction(Scene):
    def construct(self):
        ...
        title = VGroup(
            Tex("Divisão", color=LIGHT_BLUE_COLOR).scale(2),
            Tex("de Frações", color=LIGHT_PURPLE_COLOR).scale(2),
        ).arrange(DOWN, buff=0.4)
        fraction_1 = MathTex(r"\frac{1}{3}", color=LIGHT_BLUE_COLOR)
        fraction_1.scale(1.8)
        fraction_1.shift(2.7 * DOWN + 2.5 * LEFT)
        fraction_2 = MathTex(r"\frac{5}{8}", color=LIGHT_BLUE_COLOR)
        fraction_2.scale(1.8)
        fraction_2.shift(2.7 * UP + 2.5 * RIGHT)
        fraction_3 = MathTex(r"\frac{4}{7}", color=LIGHT_PURPLE_COLOR)
        fraction_3.scale(1.8)
        fraction_3.shift(2.7 * UP + 2.5 * LEFT)
        fraction_4 = MathTex(r"\frac{2}{9}", color=LIGHT_PURPLE_COLOR)
