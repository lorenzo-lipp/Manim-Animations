...
class SequenceTwo(Scene):
    def construct(self):
        self.camera.background_color = BACKGROUND_COLOR

        animals = [Toucan, Sloth, Crab, Alligator]
        sequence = Group(*[animals[i % 4]().scale(0.7) for i in range(10)])
        sequence.arrange(DOWN, buff=0.3)
        sequence.shift(0.5 * DOWN)
        missing = sequence[6]
