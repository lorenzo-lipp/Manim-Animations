...
class DivisionBySix(Scene):
    def construct(self):
        ...
        kids = Group(
            ...
            ImageMobject("./assets/crianca-4.png"),
            ImageMobject("./assets/crianca-5.png"),
            ImageMobject("./assets/crianca-6.png")
        )
        kids.scale(0.6)
        kids.arrange(DOWN, buff=0.7)
        Group(kids, bananas).arrange(RIGHT, buff=2)
        division = Division(6, 6)
        division.scale(1.8)
        ...
        self.play(
            LaggedStart(
                ...
                Transform(bananas[1], banana.copy().scale(0.8).next_to(kids[4], buff=1)),
                Transform(bananas[0], banana.copy().scale(0.8).next_to(kids[5], buff=1)),
                run_time=2.5,
                lag_ratio=0.5
            )
        )
